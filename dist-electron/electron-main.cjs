  ...
  for (const type of DOC_TYPES) {
    ...
    try {
      ...
      while (attempts < maxAttempts && !isVerified) {
        ...
        let content2 = rawContent;
        if (type.key === "cv") {
          content2 = content2.replace(/```json/gi, "").replace(/```/g, "").trim();
          content2 = await validateAndFixCVLanguage(content2, targetLanguage, callAI2, thinker);
        } else {
          content2 = cleanAIOutput(rawContent);
          content2 = await ensureTargetLanguageOrRetry({
            content: content2,
            lang3,
            targetLanguage,
            callAI: callAI2,
        ...
        if (type.key === "motivation_letter" || type.key === "cover_letter") {
          content2 = stripLetterGreetingAndClosing(content2, isGerman);
        }
        if (auditor) {
