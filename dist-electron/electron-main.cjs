...
async function analyzeDocument(docId, userId) {
  try {
    ...
    await logAction(userId, "ai_librarian", `\u{1F4DA} Librarian is reading: ${doc.file_name}`, "in_progress");
...
async function analyzeJobUrl(jobId, userId, url2) {
  try {
    const models = await getAllQuery("SELECT * FROM ai_models");
    ...
    await logAction(userId, "ai_hunter", `\u{1F50D} Hunter is analyzing URL: ${url2}`, "in_progress");
    let html = "";
    try {
    ...
    const prompt = `Analyze this job listing URL and content: ${url2}. 
    Content Snippet: ${typeof html === "string" ? html.substring(0, 8e3) : "No content"} 
    ...
    try {
      ...
      await runQuery("UPDATE job_listings", { id: jobId, status: "manual_review", date_imported: (/* @__PURE__ */ new Date()).toLocaleDateString() });
...
async function callAI(model, prompt, fileData) {
  ...
  try {
    if (!model || !model.api_key)
      return "Error: Missing API Key for " + ((model == null ? void 0 : model.role) || "Unknown Role");
    ...
    const isImage = fileData == null ? void 0 : fileData.startsWith("data:image");
    const messages = [];
    ...
    const response = await axios_default.post(endpoint, {
      model: modelName,
      messages,
    ...
    const errMsg = ((_c = (_b = (_a = err.response) == null ? void 0 : _a.data) == null ? void 0 : _b.error) == null ? void 0 : _c.message) || err.message;
    return "Error: " + errMsg;
  }
}
async function logAction(userId, type, desc, status, success) {
  await runQuery("INSERT INTO action_logs", { user_id: userId, action_type: type, action_description: desc, status, success });
}
async function processApplication(jobId, userId) {
  try {
    ...
    await logAction(userId, "ai_thinker", `\u{1F914} Thinker evaluating: ${job.job_title}`, "in_progress");
    ...
    await logAction(userId, "ai_auditor", `\u{1F50D} Auditor checking for Ghost Job: ${job.company_name}`, "in_progress");
    ...
    while (!approved && attempts < 3) {
      ...
      if (thinker.cv_style_persona === "Mimic my CV" && thinker.reference_cv_id) {
        const refCv = db.documents.find((d) => d.id === parseInt(thinker.reference_cv_id));
        if (refCv)
          genPrompt += `
...
Apply this CV Style Code: ${thinker.cv_style_code}`;
      ...
      await logAction(userId, "ai_auditor", `\u{1F9D0} Auditor reviewing materials for accuracy and ATS compatibility...`, "in_progress");
    ...
    await logAction(userId, "ai_observer", `\u{1F440} Roadblock detected! Waiting for user permission to submit.`, "waiting");
    return { success: true };
  } catch (error) {
    return { success: false, error: error.message };
  }
}
async function solveRoadblock(jobId, userId) {
  try {
    const models = await getAllQuery("SELECT * FROM ai_models");
    const observer = models.find((m) => m.role === "Observer" && m.status === "active");
    const mouse = models.find((m) => m.role === "AI Mouse" && m.status === "active");
    if (!observer || !mouse)
      return { success: false, error: "Observer or AI Mouse missing" };
    await logAction(userId, "ai_observer", `\u{1F4F8} Observer taking screenshot...`, "in_progress");
    ...
    await logAction(userId, "ai_mouse", `\u{1F5B1}\uFE0F AI Mouse executing: ${action}`, "in_progress");
...
function setupIpcHandlers() {
  ...
  import_electron2.ipcMain.handle("ai:handle-intervention", async (_, jobId, userId, allowAI) => {
    if (allowAI)
      return await solveRoadblock(jobId, userId);
    await runQuery("UPDATE job_listings", { id: jobId, needs_user_intervention: 0, status: "manual" });
    return { success: true };
  });
  import_electron2.ipcMain.handle("ai:fetch-models", async (_, apiKey, role) => await (void 0)(apiKey, role));
  ...
  import_electron2.ipcMain.handle("jobs:add-manual", async (_, data) => {
    ...
    if (existing.some((j) => j.url === data.url))
    ...
    analyzeJobUrl(result.id, data.userId, data.url).catch(console.error);
    return { success: true, id: result.id };
  });
  import_electron2.ipcMain.handle("jobs:delete", async (_, id) => {
  ...
  import_electron2.ipcMain.handle("questions:get-all", async (_, userId) => ({ success: true, data: await getAllQuery("SELECT * FROM questions") }));
  import_electron2.ipcMain.handle("questions:answer", async (_, data) => await runQuery("UPDATE questions", { ...data, status: "answered" }));
}
