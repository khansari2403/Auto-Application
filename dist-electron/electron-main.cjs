...
__export(doc_generator_exports, {
  ...
  determineJobLanguageUsingLLM: () => determineJobLanguageUsingLLM,
  ensureTargetLanguageOrRetry: () => ensureTargetLanguageOrRetry,
  generateCVHTML: () => generateCVHTML,
  generateCompanyDeepDive: () => generateCompanyDeepDive,
...
${raw.substring(0, 8e3)}`;
    ...
    const targetLanguage = language in iso6393ToLanguageName ? language : iso6393ToLanguageName[lang3] || language || "ENGLISH";
...
function formatEducationsForPrompt(educations) {
...
function generateCVHTML(content, userProfile, job, isGerman, targetLanguage, cvStylePersona) {
  ...
  const labels = {
    ...
    FRENCH: { summary: "Profil Professionnel", experience: "Exp\xE9rience Professionnelle", education: "Formation", skills: "Comp\xE9tences", certifications: "Certifications", languages: "Langues", present: "Pr\xE9sent" },
    SPANISH: { summary: "Perfil Profesional", experience: "Experiencia Laboral", education: "Educaci\xF3n", skills: "Habilidades", certifications: "Certificaciones", languages: "Idiomas", present: "Presente" },
  ...
  const sidebarLabels = {
    ...
    FRENCH: { contact: "Contact", extras: "Comp\xE9tences", certs: "Certifications" },
  ...
  const htmlLangMap = { GERMAN: "de", ENGLISH: "en", FRENCH: "fr", SPANISH: "es", ITALIAN: "it", DUTCH: "nl" };
  ...
  const sortDesc = (a, b) => {
    ...
    const isPresent = (d) => /present|heute|now|current|bis heute|seit|since|ongoing|laufend/i.test(String(d || ""));
    ...
    if (a.current && b.current || presentA && presentB) {
      const startA2 = a.startDate || a.start_date || a.from || a.start || a.startYear || a.start_year || "";
      const startB2 = b.startDate || b.start_date || b.from || b.start || b.startYear || b.start_year || "";
      return getYear(startB2) - getYear(startA2);
    }
  ...
  const renderExperiences = () => {
    let exps = (userProfile == null ? void 0 : userProfile.experiences) || [];
    if (!Array.isArray(exps) || exps.length === 0) return "";
    const expsWithIdx = exps.map((e, i) => ({ ...e, _originalIndex: i }));
    expsWithIdx.sort(sortDesc);
    return expsWithIdx.map((exp, idx) => {
      var _a;
      const originalIdx = exp._originalIndex;
      const rawDesc = ((_a = rewritten.experiences) == null ? void 0 : _a[String(originalIdx)]) || exp.description || exp.details || "";
      const desc = rawDesc.replace(/^<ul>/, '<ul class="exp-list">');
  ...
  const renderEducations = () => {
    let edus = (userProfile == null ? void 0 : userProfile.educations) || [];
    if (!Array.isArray(edus) || edus.length === 0) return "";
    const edusWithIdx = edus.map((e, i) => ({ ...e, _originalIndex: i }));
    edusWithIdx.sort(sortDesc);
    return edusWithIdx.map((edu, idx) => {
      var _a;
      const originalIdx = edu._originalIndex;
      const rawDesc = ((_a = rewritten.educations) == null ? void 0 : _a[String(originalIdx)]) || edu.details || edu.description || "";
      const desc = rawDesc;
  ...
  const summaryText = rewritten.summary || (userProfile == null ? void 0 : userProfile.summary) || "";
  if (isMimicPersona) {
...
<head>
  <meta charset="UTF-8">
  <title>${lang === "GERMAN" ? "Lebenslauf" : lang === "FRENCH" ? "CV" : lang === "SPANISH" ? "Curr\xEDculum" : "CV"} - ${(userProfile == null ? void 0 : userProfile.name) || (lang === "GERMAN" ? "Bewerber" : "Applicant")}</title>
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap');
    * { margin: 0; padding: 0; box-sizing: border-box; }
...
async function buildCompanyDeepDive(job, userId, callAI2) {
  const db = getDatabase();
  const models = await getAllQuery("SELECT * FROM ai_models");
  const detectiveModel = models.find((m) => m.role === "Detective" && m.status === "active");
  const thinkerModel = models.find((m) => m.role === "Thinker" && m.status === "active");
  ...
  const depth = deepDiveLevel === "light" || deepDiveLevel === "deep" ? deepDiveLevel : "normal";
  let scrapedInfo = "";
  try {
    await logAction(userId, "ai_detective", `\u{1F50D} Detective researching ${job.company_name}...`, "in_progress");
    scrapedInfo = await getCompanyInfo(job.company_name, userId, callAI2);
  } catch (e) {
    console.error("Detective research failed:", (e == null ? void 0 : e.message) || e);
  }
  const jobContext = `Job Title: ${job.job_title || "N/A"}
Company: ${job.company_name || "N/A"}
Location: ${job.location || "N/A"}
Summary from job ad: ${(job.description || "").substring(0, 1e3)}`;
  const functionalPrompt = model.functional_prompt || "";
  const depthInstruction = depth === "light" ? "Keep each section extremely short (1-2 concise sentences)." : depth === "deep" ? "Provide rich but focused detail for each section (2 short paragraphs max)." : "Provide a balanced level of detail for each section (3-5 sentences).";
...
${scrapedInfo || "No additional info scraped."}`;
  ...
  let cultureValues = "";
  try {
    const raw = await callAI2(model, prompt);
    ...
    console.error("Failed to parse Detective deep dive response:", (e == null ? void 0 : e.message) || e);
  }
  const combinedRaw = [scrapedInfo, missionVision, productsServices, targetMarkets, cultureValues].filter(Boolean).join("\n\n");
  return {
    missionVision,
    productsServices,
    targetMarkets,
...
async function generateCompanyDeepDive(job, userId, callAI2) {
  const deepDive = await buildCompanyDeepDive(job, userId, callAI2);
  try {
    await runQuery("UPDATE job_listings", {
      id: String(job.id),
    ...
    console.error("Failed to persist company deep dive:", (e == null ? void 0 : e.message) || e);
  }
  return deepDive;
}
async function translateCVContent(jsonContent, targetLanguage, callAI2, thinker) {
  try {
    const parsed = JSON.parse(jsonContent);
    if (parsed.summary && typeof parsed.summary === "string" && parsed.summary.length > 0) {
      const translatedSummary = await translateText(parsed.summary, targetLanguage, callAI2, thinker);
    ...
    if (parsed.experiences && typeof parsed.experiences === "object") {
      for (const key of Object.keys(parsed.experiences)) {
        ...
        if (typeof expText === "string" && expText.length > 0) {
          const translated = await translateText(expText, targetLanguage, callAI2, thinker);
    ...
    if (parsed.educations && typeof parsed.educations === "object") {
      for (const key of Object.keys(parsed.educations)) {
        ...
        if (typeof eduText === "string" && eduText.length > 0) {
          const translated = await translateText(eduText, targetLanguage, callAI2, thinker);
...
async function translateDocumentContent(content, targetLanguage, callAI2, thinker) {
  return await translateText(content, targetLanguage, callAI2, thinker);
}
...
async function generateTailoredDocs(job, userId, thinker, auditor, options, callAI2) {
  const db = getDatabase();
  const { isGerman, targetLanguage, lang3 } = await determineJobLanguageUsingLLM(job, thinker, callAI2);
  ...
  let companyResearch = "";
  let companyDeepDive = null;
  try {
    const settings = db.settings && db.settings[0] || {};
    const deepAutoDiveMode = settings.deep_auto_dive_mode || "off";
    const compatScore = job.compatibility_score || 0;
    const isGoldJob = compatScore >= 76;
    const isGreenJob = compatScore >= 51 && compatScore < 76;
    const isYellowJob = compatScore >= 26 && compatScore < 51;
    ...
    if (isEligibleForDeepDive) {
      companyDeepDive = await buildCompanyDeepDive(job, userId, callAI2);
  ...
  for (const type of DOC_TYPES) {
    ...
    try {
      ...
      while (attempts < maxAttempts && !isVerified) {
        attempts++;
        if (attempts > 1) {
          await logAction(userId, "ai_auditor", `\u{1F504} Auditor requested changes. Retrying (${attempts}/${maxAttempts})...`, "in_progress");
        }
        ...
        await logAction(userId, "ai_thinker", `\u{1F4DD} Step 1/3: Generating base content...`, "in_progress");
        const rawContent = await callAI2(thinker, basePrompt);
        ...
        let content2 = baseContent;
        if (targetLanguage !== "ENGLISH") {
          await logAction(userId, "ai_thinker", `\u{1F30D} Step 2/3: Translating to ${targetLanguage}...`, "in_progress");
          if (type.key === "cv") {
            content2 = await translateCVContent(baseContent, targetLanguage, callAI2, thinker);
          } else {
            content2 = await translateDocumentContent(baseContent, targetLanguage, callAI2, thinker);
          }
        } else {
          await logAction(userId, "ai_thinker", `\u2713 Step 2/3: Target language is English, skipping translation`, "info");
        }
        await logAction(userId, "ai_auditor", `\u{1F50D} Step 3/3: Running accuracy verification...`, "in_progress");
        if (auditor) {
          const auditorPrompt = buildVerificationPrompt(type.key, type.label, content2, filteredProfile);
          const auditorResponse = await callAI2(auditor, auditorPrompt);
...
Mission & Vision: ${(companyDeepDive == null ? void 0 : companyDeepDive.missionVision) || "N/A"}
Products & Services: ${(companyDeepDive == null ? void 0 : companyDeepDive.productsServices) || "N/A"}
Target Customers / Markets: ${(companyDeepDive == null ? void 0 : companyDeepDive.targetMarkets) || "N/A"}
Culture & Values: ${(companyDeepDive == null ? void 0 : companyDeepDive.cultureValues) || "N/A"}
...
- For EACH education entry, use a similar pattern, but with STRICT order and no duplicate labels:
  ...
  5) If there are education details (thesis, focus, courses), use ONE line starting with "Aufgaben:" / "T\xE4tigkeiten:" / "Responsibilities:" and then list them. Do NOT repeat "Aufgaben" twice for the same education entry.
...
function registerUserHandlers() {
  ...
  import_electron3.ipcMain.handle("user:capture-linkedin", async (_, data) => {
    try {
      const LinkedInScraper = (init_linkedin_scraper(), __toCommonJS(linkedin_scraper_exports));
      const { getAllQuery: getAllQuery2 } = (init_database(), __toCommonJS(database_exports));
      const aiService = (init_ai_service(), __toCommonJS(ai_service_exports));
      const { userId, profileUrl, useAI } = data || {};
      let hunterModel = null;
      if (useAI !== false) {
        try {
          const models = await getAllQuery2("SELECT * FROM ai_models");
...
function registerAIHandlers() {
  ...
  import_electron6.ipcMain.handle("ai:company-deep-dive", async (_, data) => {
    var _a;
    try {
      ...
      const deepDive = await DocGenerator.generateCompanyDeepDive(job, userId, callAI);
  ...
  import_electron6.ipcMain.handle("ai:detect-job-language", async (_, payload) => {
    var _a;
    try {
      ...
      const models = await getAllQuery("SELECT * FROM ai_models");
      const thinker = models.find((m) => m.role === "Thinker" && m.status === "active");
      const { isGerman, targetLanguage, lang3, confidence } = await DocGenerator.determineJobLanguageUsingLLM(job, thinker, callAI);
      const upperLang = String(targetLanguage || "").toUpperCase();
      let allowedLanguages = [];
      try {
        const searchProfiles = db.search_profiles || [];
        const activeProfile = searchProfiles.find((p) => p.is_active === 1) || searchProfiles[0];
        if (activeProfile == null ? void 0 : activeProfile.languages) {
          allowedLanguages = String(activeProfile.languages).split(",").map((s) => s.trim()).filter(Boolean).map((s) => s.toUpperCase());
        }
      } catch (e) {
      ...
      const languageAliases = {
        ...
        FRENCH: ["FRENCH", "FRANZ\xD6SISCH", "FRANCAIS", "FR"],
...
function registerAIModelsHandlers() {
  ...
  import_electron9.ipcMain.handle("ai-models:update", async (_, data) => {
    try {
      ...
      if (data.deepDiveLevel !== void 0) dbData2.deep_dive_level = data.deepDiveLevel;
      if (data.deep_dive_level !== void 0) dbData2.deep_dive_level = data.deep_dive_level;
