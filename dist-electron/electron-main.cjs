...
async function processApplication(jobId, userId) {
  try {
    ...
    await logAction(userId, "ai_loop", `\u{1F680} Team Assembled: ${thinker.model_name} & ${auditor.model_name}`, "in_progress");
    await logAction(userId, "ai_thinker", `\u{1F9E0} ${thinker.model_name} is researching and writing...`, "in_progress");
    const thinkerPrompt = `
    ...
    await logAction(userId, "ai_auditor", `\u{1F9D0} ${auditor.model_name} is auditing the work...`, "in_progress");
    const auditorPrompt = `
    ...
    import_fs2.default.writeFileSync(import_path2.default.join(saveDir, "Tailored_Application.txt"), thinkerResponse + "\n\n--- AUDIT REPORT ---\n" + auditResult);
...
async function callAI(model, prompt) {
  var _a, _b, _c;
  try {
    const response = await axios_default.post("https://api.openai.com/v1/chat/completions", {
    ...
    throw new Error(`API Error (${model.model_name}): ${((_c = (_b = (_a = err.response) == null ? void 0 : _a.data) == null ? void 0 : _b.error) == null ? void 0 : _c.message) || err.message}`);
  }
}
