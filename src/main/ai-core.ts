...
import { checkReputation } from './features/ghost-job-network';
import { generateTailoredDocs } from './features/doc-generator';
...
export async function processApplication(jobId: number, userId: number, callAI: Function, userConsentGiven: boolean = false) {
  try {
    ...
    if (thinker && auditor) {
      ...
      await generateTailoredDocs(job, userId, thinker, auditor, mockOptions, callAI);
    }
    return { success: true };
  } catch (error: any) {
    return { success: false, error: error.message };
