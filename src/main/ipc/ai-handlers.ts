...
Description: ${job.description || 'Not available'}`;
        ...
        try {
          ...
          console.log('Scraper result:', pageData?.strategyUsed, 'Content length:', pageData?.content?.length || 0);
      ...
      // Validate we have enough job context
...
[
  ...
  // Get answered questions for the archive
  ipcMain.handle('auditor:get-answered-questions', async (_, data) => {
    try {
      const { userId } = data;
      const db = getDatabase();
      const questions = db.auditor_questions || [];
      const answered = questions.filter((q: any) => q.user_id === userId && q.answered);
      
      // Get the answers from auditor_criteria to show in archive
      const criteria = db.auditor_criteria || [];
      const questionsWithAnswers = answered.map((q: any) => {
        const crit = criteria.find((c: any) => c.criteria === q.criteria && c.user_id === userId);
        return {
          question: q,
  ...
  ipcMain.handle('auditor:get-learned-criteria', async (_, data) => {
    try {
      ...
      return { success: true, criteria: userCriteria.map((c: any) => ({ ...c, userAnswer: c.user_answer || c.userAnswer })) };
    } catch (e: any) {
      return { success: false, error: e.message };
    }
  });
  ...
  ipcMain.handle('auditor:save-criteria', async (_, data) => {
    try {
      ...
      // Mark the question as answered (find by ID and update)
      if (questionId) {
  ...
  ipcMain.handle('auditor:update-criteria', async (_, data) => {
    try {
      ...
      // Use consistent field name: userAnswer (matching save-criteria handler)
